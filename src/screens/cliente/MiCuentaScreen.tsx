...
const MiCuentaScreen: React.FC = () => {
  ...
  const rows = [
    ...
    { label: 'Rol',     val: 'Cliente' },
    { label: 'Miembro desde', val: usuario?.created_at ? new Date(usuario.created_at).toLocaleDateString('es-MX') : 'N/A' },
  ];
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.avatarArea}>
        <View style={[styles.avatar, { borderColor: Violet.primary + '55' }]}>
          <Text style={styles.avatarEmoji}>👤</Text>
        </View>
        <Text style={styles.nombre}>{usuario?.nombre}</Text>
        <Text style={styles.correo}>{usuario?.correo}</Text>
        <View style={[styles.rolBadge, { backgroundColor: Violet.subtle, borderColor: Violet.primary + '55' }]}>
          <Text style={[styles.rolText, { color: Violet.light }]}>✨ Cliente</Text>
        </View>
      </View>
